#!/usr/bin/env python3
"""Evaluate the official pretrained OverlapTransformer without fine-tuning.
...
EXPECTED_WEIGHTS_SHA256 = (
    "5b1da14c09e990257a41b05460cc390c7c62ddb65b0bc8900d421a78a2864bae"
)
...
def prepare_cloud(
    ...
    result = points.copy()
    if up is not None:
        result[:, :3] = result[:, :3] @ gravity_canonical_rotation(up).T
...
def load_official_model(args: argparse.Namespace):
    sys.path[:0] = [str(args.ot_root), str(args.ot_root / "tools")]
    import torch
    ...
    except TypeError:  # PyTorch 1.x, used by the upstream release.
        checkpoint = torch.load(args.weights, map_location="cpu")
    model = featureExtracter(channels=1, use_transformer=True)
    model.load_state_dict(checkpoint["state_dict"])
    model.eval()
    return torch, model, range_projection


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--ot-root", required=True, type=Path)
    ...
    parser.add_argument("--fov-up", type=float, default=3.0)
    parser.add_argument("--fov-down", type=float, default=-25.0)
    parser.add_argument("--projection-max-range", type=float, default=80.0)
    ...
    if weights_hash != EXPECTED_WEIGHTS_SHA256 and not args.allow_weight_mismatch:
        raise RuntimeError(
            "Unexpected OverlapTransformer weights SHA-256: "
    ...
    torch, model, range_projection = load_official_model(args)

    def descriptor(points: np.ndarray, up: np.ndarray | None) -> np.ndarray:
        points = prepare_cloud(
            points, up, args.min_radius, args.max_radius
        )
        projected, _, _, _ = range_projection(
            points,
            fov_up=args.fov_up,
    ...
    summary = {
        ...
        "protocol": args.protocol + "_single_scan_kitti_pretrained_zero_shot",
    ...
    try:
        ...
        commit = subprocess.run(
            ["git", "-C", str(args.ot_root), "rev-parse", "HEAD"],
            check=True,
            capture_output=True,
            text=True,
        ).stdout.strip()
        ...
        commit = "unavailable"
    manifest = {
        "official_repository": "https://github.com/haomo-ai/OverlapTransformer",
