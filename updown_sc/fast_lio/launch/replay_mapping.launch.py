...
def _launch_setup(context, *args, **kwargs):
    ...
    manual_loop_session_dir = LaunchConfiguration('manual_loop_session_dir')
    ...
    manual_loop_session_dir_value = manual_loop_session_dir.perform(context).strip()
    if manual_loop_session_dir_value:
        extra_params['manual_loop_export.session_dir'] = manual_loop_session_dir_value
...
def generate_launch_description():
    ...
    ld.add_action(DeclareLaunchArgument(
        ...
        description='Start ros2 bag play together with mapping'
    ))
    ld.add_action(DeclareLaunchArgument(
    ...
    ld.add_action(DeclareLaunchArgument(
        'manual_loop_session_dir', default_value='',
        description='Manual-loop session output directory override'
    ))
    ld.add_action(DeclareLaunchArgument(
