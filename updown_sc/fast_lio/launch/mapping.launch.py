...
def _launch_setup(context, *args, **kwargs):
    ...
    mapping_rviz_config_path = os.path.join(
    ...
    if not rviz_cfg_value:
        rviz_cfg_value = mapping_rviz_config_path
    if rviz_enabled and not os.path.exists(rviz_cfg_value):
        raise RuntimeError(f'rviz_cfg does not exist: {rviz_cfg_value}')
    record_bag_enabled = _as_bool(record_bag.perform(context).strip(), True, 'record_bag')
    bag_output_dir_value = bag_output_dir.perform(context).strip()
    if not bag_output_dir_value:
        bag_output_dir_value = os.path.join(os.getcwd(), 'fast_lio', 'mapping_rosbag')
    ...
    if record_bag_enabled:
        common = ros_parameters.get('common', {})
        if not isinstance(common, dict):
            raise RuntimeError(f'Config common must be a YAML mapping: {resolved_config_file}')
        record_topics = []
        for topic in (
            str(common.get('lid_topic', '/livox/lidar')).strip(),
            str(common.get('imu_topic', '/livox/imu')).strip(),
            str(common.get(
                'front_lid_topic', '/driver/lidar/lidar_front/point_cloud/Data')).strip(),
        ):
            if topic and topic not in record_topics:
                record_topics.append(topic)
        if not record_topics:
            raise RuntimeError('No input topics configured for ros2 bag record')
        os.makedirs(bag_output_dir_value, exist_ok=True)
        bag_output = os.path.join(
            bag_output_dir_value,
            'mapping_' + datetime.now().strftime('%Y%m%d_%H%M%S'))
        qos_path = os.path.join(os.path.dirname(resolved_config_file), 'rosbag_qos.yaml')
        record_cmd = ['ros2', 'bag', 'record', '--output', bag_output]
        if os.path.exists(qos_path):
            record_cmd += ['--qos-profile-overrides-path', qos_path]
        record_cmd += ['--topics'] + record_topics
...
def generate_launch_description():
    ...
    declare_record_bag_cmd = DeclareLaunchArgument(
        ...
        description='Record mapping input LiDAR/IMU/front LiDAR topics from config'
    )
    declare_bag_output_dir_cmd = DeclareLaunchArgument(
        'bag_output_dir', default_value='',
        description='Directory for mapping rosbag output'
    )
    ...
    ld.add_action(declare_bag_output_dir_cmd)
